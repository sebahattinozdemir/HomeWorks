...
(void) fprintf(ptr, "P6\n%d %d\n255\n", image.width, image.height);
 ...
 fwrite(image.pixels,1, image.width*image.height*3,  ptr);
...
)
{
  ...
  /* ...  */
         ...
         endAngleDegrees  = endAngleDegrees-90;
      
		 arc = endAngleDegrees - startAngleDegrees;
		
	   	 px = center.col + radius * cos(  startAngleDegrees * PI*2 / res ) ;
		 py = center.row + radius * sin(  startAngleDegrees * PI*2 / res ) ;	

		 start= makePoint(py,px);

         startAngleDegrees+=(arc/numberOfSegments);
	        
                 
	        for(j=0;j<numberOfSegments-1;j++){
		    

	         px = center.col + radius * cos(  startAngleDegrees * PI*2 / res ) ;
			 py = center.row + radius * sin(  startAngleDegrees * PI*2 / res ) ;	

		     drawLine(image,start,makePoint(py, px),color);
...
Boolean drawRectangle ( Image image , Point topLeft , unsigned short int width , unsigned short int height , Color color )
{
 ...
 topRight = makePoint(topLeft.row,topLeft.col+width); 
 
 drawLine(image,topLeft,topRight,color);
 
 bottomLeft = makePoint(topLeft.row+height,topLeft.col);
 drawLine(image,bottomLeft,topLeft,color); 
 
 bottomRight = makePoint(topRight.row+height,topLeft.col+width);
 drawLine(image,bottomRight,topRight,color);
   
 drawLine(image,bottomRight,bottomLeft,color);  
...
Boolean drawRegularConvexPolygon ( Image image , Point center , unsigned short int radius , unsigned short int numberOfSides , Color color )
{
  ...
  /* ...  */
     ...
     double angle_step,x,y;
     Point start;
     int i;
 
     angle_step = 2.0 * PI /(double) numberOfSides;
     x = center.col + radius*cos(angle_step);
     y = center.row + radius*sin(angle_step);

      start = makePoint(y,x);
     
   for (i=2; i<numberOfSides+2; ++i)
   {
      double angle = angle_step * i;

      double x = center.col + radius*cos(angle);
      double y = center.row + radius*sin(angle);

        drawLine(image,start,makePoint(y,x),color);
...
Boolean drawCharacter ( Image image , Point topLeft , char c , Color color )
{
  ...
  /* ...  */
 	...
 	Point initialrow = topLeft;
 	for(i=12;i>=0;i--){

   	unsigned char hexadecimal_number = characterRasters[(c-32)][i];
  
    char *bin1,*bin2;
	if((hexadecimal_number/16)==0)
		 bin1 = "0000";
	else if((hexadecimal_number/16)==1)
		 bin1 = "0001";
	else if((hexadecimal_number/16)==2)
		 bin1 = "0010";
	else if((hexadecimal_number/16)==3)
		 bin1 = "0011";
	else if((hexadecimal_number/16)==4)
		 bin1 = "0100";
	else if((hexadecimal_number/16)==5)
		 bin1 = "0101";
	else if((hexadecimal_number/16)==6)
		 bin1 = "0110";
	else if((hexadecimal_number/16)==7)
		 bin1 = "0111";
	else if((hexadecimal_number/16)==8)
		 bin1 = "1000";
	else if((hexadecimal_number/16)==9)
		 bin1 = "1001";
	else if((hexadecimal_number/16)==10)
		 bin1 = "1010";
	else if((hexadecimal_number/16)==11)
		 bin1 = "1011";
	else if((hexadecimal_number/16)==12)
		 bin1 = "1100";
	else if((hexadecimal_number/16)==13)
		 bin1 = "1101";
	else if((hexadecimal_number/16)==14)
		 bin1 = "1110";
	else if((hexadecimal_number/16)==15)
		 bin1 = "1111";
   ...
   initialrow.col = topLeft.col;

   for(k = 0 ; k < 4 ; k++ ){
		if(bin1[k] == '1'){
      		setPixel(image,initialrow,color);
